...
void UartDeviceInit(void) {
    ...
    for (size_t i = 0; i < UartChannelMax; i++) {
        ...
        DevUartDMARecive(&(UartBspCfg[i].uart_cfg), uart_handle->rx_buffer, UartBspCfg[i].buffer_size);
...
void UartStreamRcvTask(void *arg) {
    ...
    while (1) {
        rcv_count = xStreamBufferReceive(uart_handle->rx_stream_buffer, uart_handle->rx_buffer_for_vfb, uart_handle->buffer_size, portMAX_DELAY);
        if (rcv_count > 0) {
