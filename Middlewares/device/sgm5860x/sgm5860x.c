...
/* ===================================================================================== */
#define ARRAYSIZE 2
uint8_t sgm5860x_send_buffer[sgm5860xChannelMax][ARRAYSIZE] = {};
typedef struct
{
    uint32_t ch;
...
const Typdefsgm5860xBSPCfg sgm5860xBspCfg[sgm5860xChannelMax] = {
    {
        .ch   = 0,
        .drdy = {
        ...
        .nest = {
            .device_name = "sgm5860x_nest", .base = GPIOD, .af = 0, .pin = GPIO_PIN_10, .pin_mode = DevPinModeOutput,
            .bit_value = 1,  // Active low
        },
        .sync = {
            .device_name = "sgm5860x_sync", .base = GPIOD, .af = 0, .pin = GPIO_PIN_11, .pin_mode = DevPinModeOutput,
            .bit_value = 1,  // Active low
        },
        .spi_cfg = {
            .base = SPI1,
            // SPI configuration
            .nss = {
                .device_name = "sgm5860x_nss", .base = GPIOB, .af = 0, .pin = GPIO_PIN_12, .pin_mode = DevPinModeOutput,
                .bit_value = 1,  // Active low
            },
            .clk = {
...
void sgm5860xDeviceInit(void) {
    ...
    for (size_t i = 0; i < sgm5860xChannelMax; i++) {
        ...
        DevSpiHandleStruct *spicfg                 =( DevSpiHandleStruct *) &sgm5860xBspCfg[i].spi_cfg;
        sgm5860xStatusHandle->id                   = i;
        memset(sgm5860xStatusHandle->device_name, 0, sizeof(sgm5860xStatusHandle->device_name));
        snprintf(sgm5860xStatusHandle->device_name, sizeof(sgm5860xStatusHandle->device_name), "sgm5860x%d", i);
        sgm5860xStatusHandle->cfg = sgm5860xBspCfg[i];  // Copy configuration

        DevPinInit(&sgm5860xBspCfg[i].drdy);
        DevPinInit(&sgm5860xBspCfg[i].nest);
        DevPinInit(&sgm5860xBspCfg[i].sync);
        DevSpiInit(spicfg);

        // sgm5860xsend(i, 0x0000);
        elog_i(TAG, "sgm5860x[%d] device_name: %s, spi_base: 0x%08X", i, sgm5860xStatusHandle->device_name, sgm5860xStatusHandle->cfg.spi_cfg.base);
    }
}
...
#endif
void sgm5860xsend(uint8_t ch, uint16_t data) {
    elog_i(TAG, "sgm5860xsend: ch=%d, data=0x%04X %d", ch, data, data);

    uint8_t i = ch;
    if (i >= sgm5860xChannelMax) {
        elog_e(TAG, "Invalid channel: %d", i);
        return;
    }
    // Prepare the data to send
    sgm5860x_send_buffer[i][0] = (data >> 8) & 0xFF;  // High byte
    sgm5860x_send_buffer[i][1] = data & 0xFF;         // Low byte
    DevSpiWriteRead(&sgm5860xBspCfg[i].spi_cfg, &(sgm5860x_send_buffer[i][0]), NULL, ARRAYSIZE);
    elog_i(TAG, "sgm5860x_send_buffer[%d][0]: 0x%02X, sgm5860x_send_buffer[%d][1]: 0x%02X", i, sgm5860x_send_buffer[i][0], i, sgm5860x_send_buffer[i][1]);
...
uint8_t sgm5860x_send_buffer_test[sgm5860x_NEW_ARRAYSIZE]    = {0xA1, 0xA2, 0xA3, 0xA4, 0xA5, 0xA6, 0xA7, 0xA8, 0xA9, 0xAA};
uint8_t sgm5860x_recv_buffer_test[sgm5860x_NEW_ARRAYSIZE] = {0xC1, 0xC2, 0xC3, 0xC4, 0xC5, 0xC6, 0xC7, 0xC8, 0xC9, 0xCA};
...
static int Cmdsgm5860xHandle(int argc, char *argv[]) {
    ...
    if (strcmp(argv[1], "test") == 0) {
        ...
        DevSpiWriteRead(&sgm5860xBspCfg[0].spi_cfg, sgm5860x_send_buffer_test, sgm5860x_recv_buffer_test, sgm5860x_NEW_ARRAYSIZE);
...
SHELL_EXPORT_CMD(SHELL_CMD_PERMISSION(0) | SHELL_CMD_TYPE(SHELL_TYPE_CMD_MAIN), sgm5860x, Cmdsgm5860xHandle, sgm5860x command);